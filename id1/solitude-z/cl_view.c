...
cvar_t	cl_rollangle = {"cl_rollangle", "2"};	// Quake default was 2
cvar_t	cl_rollalpha = {"cl_rollalpha", "20"};
cvar_t	cl_bob = {"cl_bob","0.02"};				// Quake default was 0.02
...
static qbool landed=false;
static double landedtime=0;
static qbool wasairborne=false;
static qbool dobobside=false;
...
float V_CalcBob (void)
{
	...
	if (wasairborne) {
		...
		landed = true; // Landed stays true for 0.2 seconds
		landedtime = bobtime;
	}			
	...
	if (landed) {
		if (bobtime > (landed + 0.2))
			landed = false; // Clear state it
		else
			return bob; // Still landing, no bob
	}
...
{
	static	double	bobsidetime;
	static float	bobside;
	static float	cycle;

	if (!dobobside)
		return bobside; // Same as before

	bobsidetime += cls.frametime;

	cycle = bobsidetime - (int)(bobsidetime/cl_bobsidecycle.value)*cl_bobsidecycle.value;
...
// (don't count Z, or jumping messes it up)
	...
	bobside = sqrt(cl.simvel[0]*cl.simvel[0] + cl.simvel[1]*cl.simvel[1]) * cl_bobside.value;
...
#if 1 // Baker: kurok style
cvar_t	v_centermove = {"v_centermove", "1", false};
cvar_t	v_centerspeed = {"v_centerspeed","100"};
#else // Real style
...
void V_AddIdle (void)
{
	 if (1)
    {
	    ...
	    r_refdef2.viewangles[ROLL] += v_idlescale * sin(cl.time*v_iroll_cycle) * (v_iroll_level * bspeed);
	    r_refdef2.viewangles[PITCH] += v_idlescale * sin(cl.time*v_ipitch_cycle) * (v_ipitch_level * bspeed);
	    r_refdef2.viewangles[YAW] += v_idlescale * sin(cl.time*v_iyaw_cycle) * (v_iyaw_level * bspeed);

  	    r_refdef2.viewangles[ROLL] += (v_idlescale * 3 ) * sin(cl.time*v_iroll_cycle * 0.25 ) * v_iroll_level;
	    r_refdef2.viewangles[PITCH] += (v_idlescale * 3 ) * sin(cl.time*v_ipitch_cycle * 0.25 ) * v_ipitch_level;
	    r_refdef2.viewangles[YAW] += (v_idlescale * 3 ) * sin(cl.time*v_iyaw_cycle * 0.25 ) * v_iyaw_level;
    }
    else 
    {
...
==================
*/
void V_AddViewWeapon (float bob, float bobside)
{
...
//	VectorMA (ent.origin, bob * 0.4, bobside*0.2, ent.origin);

//	VectorCopy (ent->origin, view->origin);
//	ent.origin[2] += cl.viewheight;

	{
		int i;
		for (i=0 ; i<3 ; i++)
		{
	//		ent.origin[i] += forward[i]*bob*0.4;
			ent.origin[i] += right[i]*bobside*0.2;
			ent.origin[i] += up[i]*bob*0.2;
		}
//		ent.origin[2] += bob;
	}
...
void V_Init (void)
{
	...
	Cvar_Register (&cl_bobside);
	Cvar_Register (&cl_bobsidecycle);
	Cvar_Register (&cl_bobsideup);

	Cvar_Register (&cl_gunpitch);
