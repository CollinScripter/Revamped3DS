...
static void Config_PrintPreamble(FILE *f) {
	...
	Config_PrintLine(f, "", 3);
	Config_PrintLine(f, "http://www.enginez.net/", 3);
	Config_PrintLine(f, "", 3);
	Config_PrintLine(f, "", 3);
	Config_PrintBorder(f);
	Config_PrintBorder(f);
...
void DumpConfig(char *name)	{
	...
	outfile = va("%s/enginez/configs/%s", com_basedir, name);
...
void SaveConfig_f(void)	{
	...
	if (cfg_backup.value) {
		filename_ext = va("%s/enginez/configs/%s", com_basedir, filename);
...
void LoadConfig_f(void)	{
	...
	filename = COM_SkipPath(Cmd_Argv(1));
	COM_ForceExtension(filename, ".cfg");
	if (!(f = fopen(va("%s/enginez/configs/%s", com_basedir, filename), "r"))) {
