...
void FChecks_VersionResponse(void) {
	if (Modules_SecurityLoaded())
		Cbuf_AddText (va("say enginez version %s " QW_PLATFORM ":" QW_RENDERER "  crc: %s\n", VersionString(), Auth_Generate_Crc()));
	else
		Cbuf_AddText (va("say enginez version %s " QW_PLATFORM ":" QW_RENDERER "\n", VersionString()));
}
...
void FChecks_FServerResponse (void) {
	...
	if (Modules_SecurityLoaded())
		Cbuf_AddText(va("say enginez f_server response: %s  crc: %s\n", NET_AdrToString(adr), Auth_Generate_Crc()));
	else
		Cbuf_AddText(va("say enginez f_server response: %s\n", NET_AdrToString(adr)));
}
...
qboolean FChecks_CheckFRulesetRequest (char *s) {
	...
	if (Util_F_Match(s, "f_ruleset"))	{
		Cbuf_AddText(va("say enginez Ruleset: %s\n", Rulesets_Ruleset() ));
