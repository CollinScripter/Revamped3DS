...
#ifdef _WIN32
	if (!(hSecurity = LoadLibrary("enginez-security.dll"))) {
		Com_Printf("\x02" "Security module not found\n");
		goto fail;
	}
#else
	if (!(hSecurity = dlopen("./enginez-security.so", RTLD_NOW))) {
		Com_Printf("\x02" "Security module not found\n");
		goto fail;
	}
#endif
...
#else
	Q_strncpyz(binary_type, "enginez", sizeof(binary_type));
#endif
...
void QLib_MissingModuleError(int errortype, char *libname, char *cmdline, char *features) {
	...
	case QLIB_ERROR_MODULE_NOT_FOUND:
		Sys_Error(
			"enginez couldn't load the required \"%s" QLIB_LIBRARY_EXTENSION "\" library.  You must either:\n"
			"i) (recommended) download the required libraries from www.enginez.net, or\n"
